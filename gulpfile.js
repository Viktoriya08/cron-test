...
import { otfToTtf, ttfToWoff, moveFonts, fontsStyle } from "./config/gulp-tasks/fonts.js"
import { deployAll, deployJS, deployCSS, deployIMG } from "./config/gulp-tasks/ftp.js"
...
const fonts = gulp.series(otfToTtf, ttfToWoff, moveFonts, fontsStyle),
	dev = gulp.series(reset, gulp.parallel(fonts, images), gulp.parallel('viteDev', localWatch)),
	build = gulp.series(reset, fonts, gulp.parallel(fonts, images, 'viteBuild'), pathsRewrite),
	deployHTML = gulp.series(build, deployAll),
	serverDev = gulp.series(reset, fonts, gulp.parallel(fonts, images, 'viteBuild'), pathsRewrite, remoteWatch);


// Экспорт задач
export { fonts, sprite, deployAll, images };
